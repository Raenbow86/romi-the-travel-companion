...
export default function Home() {
  ...
  const [beenThere, setBeenThere] = useState("yes");
  ...
  function savePlaceReport(event: FormEvent<HTMLFormElement>) {
    ...
    const newReport: TravelerReport = {
      ...
      beenThere,
      pullThrough,
      dogFriendly: reportDogs,
      hoursSeen: hoursSeen.trim(),
      shade: "",
      wifi: "",
      vibe: "",
      notes,
    ...
    setScoutPoints((n) => n + points);
    setJustSaved({ name: newReport.name, points });
    setReportName("");
    setReportArea("");
    setReportNeeds([]);
    setReportNotes("");
      ...
      <main className="min-h-screen bg-amber-50 px-5 py-8 pb-28 text-slate-800">
        <section className="mx-auto max-w-md">
          ...
          <form onSubmit={savePlaceReport} className="mt-8 space-y-5">
                ...
                {needs.map((need) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setReportNeeds((cur) =>
                          on ? cur.filter((n) => n !== need.label) : [...cur, need.label],
                        )
                      }
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {["yes", "no"].map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setBeenThere(opt)}
                    className={`rounded-full px-3 py-2 text-sm font-bold ${
                      beenThere === opt ? "bg-teal-700 text-white" : "bg-slate-50 text-slate-700 ring-1 ring-amber-100"
                    }`}
                  >
                    {opt === "yes" ? "Yes — I went" : "No"}
                  </button>
                ))}
              </div>
            </fieldset>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {["yes", "tight", "no", "unsure"].map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setPullThrough(opt)}
                    className={`rounded-full px-3 py-2 text-sm font-bold capitalize ${
                      pullThrough === opt ? "bg-teal-700 text-white" : "bg-slate-50 text-slate-700 ring-1 ring-amber-100"
                    }`}
                  >
                    {opt}
                  </button>
            ...
            <fieldset>
              <legend className="text-sm font-bold">Dogs</legend>
              <div className="mt-2 flex flex-wrap gap-2">
                {["yes", "leash", "no", "unsure"].map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setReportDogs(opt)}
                    className={`rounded-full px-3 py-2 text-sm font-bold capitalize ${
                      reportDogs === opt ? "bg-teal-700 text-white" : "bg-slate-50 text-slate-700 ring-1 ring-amber-100"
                    }`}
                  >
                    {opt}
                  </button>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {["yes", "maybe", "no"].map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setReturnAgain(opt)}
                    className={`rounded-full px-3 py-2 text-sm font-bold capitalize ${
                      returnAgain === opt ? "bg-orange-500 text-white" : "bg-slate-50 text-slate-700 ring-1 ring-amber-100"
                    }`}
                  >
                    {opt}
                  </button>
            ...
            <fieldset>
              ...
              <button
                ...
                className={`mt-2 rounded-full px-3 py-2 text-sm font-bold ${
                  googleWrong ? "bg-orange-500 text-white" : "bg-slate-50 text-slate-700 ring-1 ring-amber-100"
                }`}
              >
          ...
          {travelerReports.length === 0 ? (
            <p className="mt-3 rounded-3xl bg-white p-4 text-sm text-slate-600">
              None yet. First report can be a place you already trust — or pin the one under your boots.
            </p>
          ) : (
            travelerReports.map((report, i) => (
              <article key={report.id || `${report.name}-${i}`} className="mt-4 rounded-3xl bg-white p-4 shadow-sm">
                <p className="text-xs font-bold text-teal-700">PENDING REVIEW</p>
                <h3 className="font-black">{report.name}</h3>
                <p className="text-sm text-slate-500">📍 {report.area}</p>
                {report.helpsWith?.length ? (
                  <p className="mt-1 text-xs text-slate-500">{report.helpsWith.join(" · ")}</p>
                ) : null}
          ...
          {flaggedPlaces.length > 0 && (
            <section className="mt-8">
              ...
              <p className="mt-1 text-sm text-slate-600">Hidden from Explore. Undo if you tapped by mistake.</p>
              {flaggedPlaces.map((flag) => (
                <article key={flag.id} className="mt-3 rounded-3xl bg-white p-4 shadow-sm">
                  ...
                  <button
                    type="button"
                    onClick={() => setFlaggedPlaces((current) => current.filter((f) => f.id !== flag.id))}
